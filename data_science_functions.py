...
def display_tsne(
    ...
    """ Display the t-sne with color for each class of illustrative var.

    X_tsne : result of tsne fit transform
    illustrative_var : list of length len(X_tsne) containing classes.
    """
    ...
    # Display the points
    if illustrative_var is None:
        ax.scatter(X_tsne[:, 0], X_tsne[:, 1], s=10, alpha=alpha)
    else:
        illustrative_var = np.array(illustrative_var)
        illustrative_values = np.unique(illustrative_var)
        n_val = len(illustrative_values)
        ...
        for n, value in enumerate(illustrative_values[::-1]):
            selected = np.where(illustrative_var == value)
            ax.scatter(
                X_tsne[selected, 0], X_tsne[selected, 1],
                alpha=alpha, label=value, color=palette[n_val - 1 - n],
                s=10
            )
        ax.set_axis_off()
...
def find_tsne_best_perplexity(
    X,
    perplexities=[15, 30, 50, 70, 85, 100], 
    labels=None, 
    tsne_init='pca'
):
    """ Display a 2x3 subplots testing 6 perplexities.

    If the input matrix is a sparse matrix,  sne_init must be changed to
    'random'.

    """
    # Plotting results for several perplexities
    fig, axes = plt.subplots(2, 3, figsize=(15,10))
    for n, (perplexity, ax) in enumerate(zip(perplexities, axes.flat)):
        print(perplexity)
        tsne = TSNE(n_components=2, perplexity=perplexity, init=tsne_init)
        X_tsne = tsne.fit_transform(X)
        display_tsne(X_tsne, ax=ax, illustrative_var=labels)
        ax.set_title(f'perplexity : {perplexity}')
        # Print legend only close to the third subplot
        if n != 2:
...
def label_similarity(df, label_name_a, label_name_b):
